...
haxball.then(async (HBInit) => {
	...
	let commands = {
		...
		social: {
			...
			roles: Role.GUEST,
			desc: `Questo comando ti mostrerà il link per entrare nei nostri Social: server Discord!`,
		...
		changenickrequest: {
			aliases: ['cnr', 'changenick'],
			roles: Role.PLAYER,
			desc: `Questo comando ti permette di cambiare nickname.
				ESEMPIO: !changenick o !cnr "nickname attuale" "nickname nuovo" motivazione `,
	...
	function calculateStadiumVariables() {
		if (checkStadiumVariable && teamRed.length + teamBlue.length > 0) {
			...
			setTimeout(() => {
				...
				let allPlayer = teamRed.concat(teamBlue).concat(teamSpec);
	...
	async function changenickCommand(player, message) {
		// Comando per inviare una richiesta di cambio nickname
			...
			// Controlla se il nuovo nickname è uguale all'attuale nickname
			if (newNick === currentNick) {
				room.sendAnnouncement("Devi scegliere un nuovo nickname diverso dal tuo.", player.id, null, 'bold');
				return Promise.resolve(false);
			}
			...
			changeRequests.push({ playerName: player.name, currentNick: currentNick, requestedNick: newNick, motivation: motivation });
	...
	async function listrequestnickCommand(player, message) {
		// Comando (solo per admin) per visualizzare tutte le richieste
	...
	async function selectchangeCommand(player, message) {
		if (player.role >= Role.ADMIN) {
			...
			let requestIndex = parseInt(message.split(" ")[1]) - 1;
			...
			if (changeRequests[requestIndex]) {
				...
				if (decision === "Accetta") {
					...
					// Aggiorna il nickname nel database dei player
					await playersDB.updateOne(
						{ nickname: currentRequest.currentNick },
						{ $set: { nickname: currentRequest.requestedNick } }
					);
					/*await playersDB.updateOne(
						{ nickname: currentRequest.currentNick },
						{ $set: { nickname: currentRequest.requestedNick } }
					);*/
					// Aggiorna il nickname nel database degli admin
					await adminsDB.updateOne(
						{ nickname: currentRequest.currentNick },
						{ $set: { nickname: currentRequest.requestedNick } }
					);
					// Aggiorna il nickname nel database delle statistiche
					await statsDB.updateOne(
						{ nickname: currentRequest.currentNick },
						{ $set: { nickname: currentRequest.requestedNick } }
					);
					// Aggiorna il nickname nelle statistiche mensili
					await monthlystatsDB.updateMany(
						{ nickname: currentRequest.currentNick },
						{ $set: { nickname: currentRequest.requestedNick } }
					);
					// Aggiorna il nickname nelle penalità
					await penaltiesDB.updateMany(
						{ nickname: currentRequest.currentNick },
						{ $set: { nickname: currentRequest.requestedNick } }
					);
					// Aggiorna il nickname nei ban
					await bansDB.updateMany(
						{ nickname: currentRequest.currentNick },
						{ $set: { nickname: currentRequest.requestedNick } }
					);
					await bansDB.updateMany(
						{ real_nick: currentRequest.currentNick },
						{ $set: { real_nick: currentRequest.requestedNick } }
					);
					// Salva nel database nickRequestsDB il vecchio e nuovo nickname
					...
					// Salva nel database nickRequestsDB il vecchio e nuovo nickname con status "Rifiutato"
					await nickRequestsDB.insertOne({
						oldNickname: currentRequest.currentNick,
						newNickname: currentRequest.requestedNick,
						status: "Rifiutato 🔴",
						motivation: currentRequest.motivation,  // Salva la motivazione
						handledBy: player.name,  // Nome dell'admin che ha gestito la richiesta
						date: getFormattedDate()
					});
					room.sendAnnouncement(`La richiesta di ${currentRequest.playerName} in [${currentRequest.requestedNick}] è stata rifiutata.`, player.id, null, 'bold', HaxNotification.CHAT);
	    	...
	    	if(penaltiesPlayer.infractions && penaltiesPlayer.infractions.length > 0) {
		      ...
		      if (penaltiesPlayer.removedInfractions && penaltiesPlayer.removedInfractions.length > 0) {
		        ...
		        lastRemovedInfractions.forEach((infraction, index) => {
		          msg += `[${index + 1}] "${infraction.type}" - da "${infraction.infractionBy}" per "${infraction.reason}" [${new Date(infraction.date).toLocaleString()}] Rimosso da: "${infraction.removedBy}" perché "${infraction.removeReason}" [${new Date(infraction.removeDate).toLocaleString()}]\n`;
		        });
		      }
	...
	async function warnCommand(player, message) { // !warn #123 Si fa autogoal
	  try {
        ...
        else throw "Il formato del comando non è valido";
        if(reason=="" || reason==undefined) throw "Errore: Motivo Warn obbligatorio. Per ulteriori informazioni, digitare !help warn";
	...
	async function readMutesCommand(player) {
		...
		if (thisAdmin != undefined) {
			thisAdmin.readMutes = !thisAdmin.readMutes
			if (thisAdmin[2]) msg = "Hai attivato la lettura dei messaggi dei player mutati."
			else msg = "Hai disattivato la lettura dei messaggi dei player mutati."
			adminsDB.updateOne({ nickname: thisAdmin.nickname, auth: thisAdmin.auth }, { $set: { readMutes: thisAdmin.readMutes } })
			typeColor = announcementColor
		}
		else msg = "Errore imprevisto."
		room.sendAnnouncement(msg, player.id, typeColor, 'bold', HaxNotification.CHAT)
	}
	...
	async function banCommand(player, message) { // !ban "GIORGIO alla" Perchè si || !ban #123 Perchè si
	    ...
	    try { /*!ban "2ms" Tossico, insulto allo staff, migliore amico di alexis
	    	...
	    	writeLog("", "Errore nel comando ban:"+ err, true);
	        if (modWebhook != '') {
	        ...
	        if(playerFromBan.id != null) room.sendAnnouncement("Errore nel comando ban:"+ err, playerFromBan.id, errorColor, "bold", 2);
	    }
	}
	...
	async function unBanCommand(player, message) {
	    try {
	    	...
	    	if(message.substring(7, 8) == '"' || message.substring(7, 8) == "'"){
	        	msgArray = message.split(message.substring(7, 8)); // [0]->"!unban" [1]->"GIORGIO alla"
		        ...
		        if(banRecord == undefined || banRecord == null ) throw "Il Giocatore non risulta tra i bannati";
	    	}
	        else throw "Il formato del comando unban non è valido";

	        // levo i ban a tutti gli pseudonimi di quel giocatore bannato
            ...
            if (banRecord) {
                ...
                query = {$or:[{"nickname": banRecord.nickname}, {"real_nick": banRecord.nickname}]};
    			const result = await bansDB.deleteMany(query);

                // Annuncia la revoca del ban nella chat del gioco
                room.sendAnnouncement(`I ban di "${banRecord.nickname}" sono stati revocati per ${result} account`, null, 0x00FF00, "bold", 2);

                if (roomWebhook != '') {
					let stringContent = `[${formatCurrentDate()}] 🛂 UNBAN (${room.getPlayerList().length}/${maxPlayers})\nBan revocato per il giocatore: **"${banRecord.nickname}" Da ${player.name} per ${result} account**`;
					fetch(roomWebhook, {
						method: 'POST',
						body: JSON.stringify({
							content: stringContent,
                ...
                // Scrivi i log
                writeLog("", "Ban revocato per il giocatore: ["+banRecord.nickname+"] per "+result+" account", true);
            } else {
                room.sendAnnouncement("Giocatore non trovato o non bannato.", player.id, 0xFF0000, "bold", 2);
            }
	    } catch (err) {
	    	writeLog("", "Errore nel comando unban:"+ err, true);
	        if (modWebhook != '') {
	        ...
	        room.sendAnnouncement("Errore nel comando unban:"+ err, player.id, errorColor, "bold", 2);
	    }
	}
	...
	async function handlePlayerChat(player, message) {
		...
		if (msgArray[0][0].includes("!")) {
			...
			if (commandName) {
				...
				if (command.roles <= player.role && (player.logged || command.roles == Role.GUEST)) {
